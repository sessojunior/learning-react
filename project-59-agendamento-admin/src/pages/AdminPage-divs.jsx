...
export default function AdminPage() {
  ...
  const professionals = [
    ...
    },
    {
      ...
      workTime: { start: "10:00", end: "18:00" },
      blockedTimes: [
        { time: "16:15", duration: "2h15", description: null }
      ]
    },
    {
      id: 7,
      ...
      workTime: { start: "10:00", end: "18:00" },
      blockedTimes: [
        { time: "12:15", duration: "2h15", description: null }
      ]
    }
  ];
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Topbar */}
      <div className="flex justify-between items-start mx-6 mt-6 h-[50px]">
      ...
      <div className="h-[calc(100vh-100px)] w-[calc(100vw-368px)] overflow-auto mx-6">

        {/* Cabeçalho da tabela */}
        <div className="table-row">
          {/* Botão de mais profissionais */}
          <div className="table-cell sticky z-50 top-0 left-0 min-w-16 max-w-16 h-16 bg-white">
            <div className="absolute top-0 left-0 w-full h-full flex justify-center items-center border-r border-r-transparent bg-white">
              <button className="bg-gray-300 px-3 py-1 rounded">→</button>
            </div>
          </div>
          {/* Profissionais */}
          {professionals.map(professional => (
            <div className="table-cell sticky z-40 top-0 left-0 min-w-56 max-w-56 h-16 bg-white">
              <div key={professional.id} className="absolute top-0 left-0 w-full h-full flex gap-2 justify-center items-center border-r border-r-transparent border-b-4 border-b-gray-200 select-none">
        ...
        {/* Tabela de horas e minuto de cada profissional */}
          {timeSlots.map((timeSlot) => {
            const minutes = timeSlot.split(":")[1]; // Pega a parte dos minutos
            return (
              <div key={timeSlot} className="table-row">
                {/* Horário */}
                <div className="table-cell sticky left-0 z-40 bg-white min-w-16 max-w-16 h-16 border-r border-r-gray-200 select-none">
                  <div className="absolute top-0 left-0 w-full h-full flex justify-end items-start border-r border-r-transparent bg-white">
                    <span className="text-sm pt-1 pr-2">
                      {/* Exibe o timeSlot se não terminar com :15 ou :45, senão exibe "-" */}
                      {minutes === "15" || minutes === "45" ? "" : timeSlot}
                    </span>
                  </div>
                </div>
                {/* Profissionais */}
                {professionals.map(professional => {
                  ...
                  return (
                    <div key={professional.id} className="table-cell relative min-w-56 max-w-56 h-16 gap-2 justify-start items-start border-r border-r-gray-200 border-b border-b-gray-200">
                      <div className="absolute top-0 left-0 w-full h-full flex justify-end items-start border-r border-r-transparent bg-white">
                        {isTimeClosed ? (
